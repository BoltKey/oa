var divPos = {x: 0, y: 0};
var lastDivPos;
var keysDown;
var lastkd;
var time = 0;
var m;
var currMap = 0;
var editor = "editor";
var play = "play";
var menu = "menu";
var editPlay = "editPlay";
var menuid = 1;
var userid = -1;
var userName = "";
var version = "0.1";
var wipeSave = true;
var osc;
var gain;
var graphics = {};
var settings = {};
var sounds = {};
var e;
var programState = menu;
var medals = [];
var scores = [];
var playerBests = [];
var totalPlayers = 0;

function navigateMenu(id) {
	menuid = id;
	programState = "menu";
	$("#help").remove();
	makeMenu();
}
function toEditor() {
	e = new Editor();
	makeMenu();
}
function toAbout() {
	navigateMenu(2);
	var a = $("<div id='help' style='color: white; font-family: arial'>Optimal Racer was created by BoltKey in 2016. I wanted to create a real-time version of that classing pen and paper game sometimes called Racetrack. <br>" + 
	"All you do is adjust acceleration of your car (or the red dot, whichever you prefer). Your task is to get to the green circle as fast as possible and fight for the top spots of the leaderboards. You will only be able to submit and compare your times if you login with Kongregate. The reasons for this are: one: I did not want to deal with secure logging in myself. two: It is more convenient for Kongregate users. <br>" +
	"Special features are jumps and turbos. The duration of jump cannot be changed and is preset. The thicker and greener the jump line is, the longer the jump will be. Turbos adjust your acceleration in the direction indicated by red line.<br>" +
	"The grass sound was generated by sfxr. The medal earn sounds is a jingle of Czech crystal wine glass with wine by freesounds.org user lmr9. The car sounds are generated by your browser (unless you use IE which can't synthetize sounds). Music is Micro Machine Man by RaceTTar from NewGrounds (link in description). <br>" +
	"Press 'Esc' anywhere to go back. Click at any time to restart level." +
	"</div>");
	a.css("position", "fixed");
	a.css("left", 70);
	a.css("top", 50);
	a.css("width", 600);
	$("body").append(a);
}

function changeSetting(name, value) {
	if (typeof(value) === 'undefined') {
		settings[name] = !settings[name];
		var a = $("#s" + name);
		a.removeClass('button-primary button-highlight button-caution button-action');
		a.addClass(settings[name] ? 'button-action' : 'button-caution');
		if (name === "music") {
			sounds.music.volume = sounds.music.volume === 0 ? 0.2 : 0;
		}
		if (name === "parts") {
			grassEmitter.behaviours[1] = new Proton.Alpha(0, 0);
			roadEmitter.behaviours[1] = new Proton.Alpha(0, 0);
		}
	}
}
function settingsColors() {
	for (var i in settings) {
		var s = settings[i];
		var a = $("#s" + i);
		a.removeClass('button-primary button-highlight button-caution button-action');
		a.addClass(s ? 'button-action' : 'button-caution');
	}
}
function main() {
	// graphics
	var names = ["bronze", "gold", "levelbutton", "platinum", "silver", "bronzesmall", "silversmall", "goldsmall", "platinumsmall", 'kongregate_white', 'orlogo'];
	for (var i in names) {
		var name = names[i];
		graphics[name] = new Image();
		graphics[name].src = "graphics/" + name + ".png";
	}
	
	//settings
	settings.gho = true;
	settings.plgho = true;
	settings.mute = false;
	settings.parts = true;
	settings.music = true;
	
	//sounds
	var names = ["bronze", "gold", "platinum", "silver", "grass", "jump"];
	for (var i in names) {
		var name = names[i];
		sounds[name] = new Audio();
		sounds[name].src = "sounds/" + name + ".wav";
	}
	sounds.music = new Audio();
	sounds.music.src = "sounds/music.mp3";
	sounds.music.volume = 0.2;
	sounds.music.loop = true;
	sounds.music.play();
	if (typeof(window.AudioContext) !== 'undefined') {  // like... fuck off IE
		var audioctx = new window.AudioContext();
		
		gain = audioctx.createGain();
		osc = audioctx.createOscillator();
		osc.connect(gain);
		gain.connect(audioctx.destination);
		gain.gain.value = 0;
		
		osc.frequency.value = 250;
		osc.type = 'sawtooth';
		
		osc.start(0);
	}
	// mouse
	var offset = $("#game").offset();
	$(document).mousemove(function(e){
    divPos = {
        x: e.pageX - offset.top,
        y: e.pageY - offset.left
		}
	})
	lastDivPos = {x: 0, y: 0};
	lastmd = 0;
	mouseDown = 0;
	document.body.onmousedown = function() { 
		mouseDown = 1;
	}
	document.body.onmouseup = function() { 
		mouseDown = 0;
	}
	
	replay = new Replay();
	
	// local save stuff
	var a = localStorage.getItem("oaversion");
	if ((a != version && wipeSave) || a == null) {
		localStorage.clear();
	}
	localStorage.setItem("oaversion", version);
	loadUserBests();
	
	//Kong
	function onLoadCompleted(){
		kongregate = kongregateAPI.getAPI();
		var name = kongregate.services.getUsername();
		var id = kongregate.services.getUserId();
		if (id !== 0) {
			entry(name);
		}
	}
	kongregateAPI.loadAPI(onLoadCompleted);
	
	//m = new Map(0);
	initProton();
	
	player = new Player();
	updateMedals();
	countPlayers();
	makeMenu();
	mainloop();
	ctx.textAlign = 'enter';
}

function newLevel(a) {
	m = new Map(a);
	currMap = a;
	replay.load(a + "best");
	m.bestTime = replay.data;
	programState = play;
	player.die();
	makeMenu();
}

function updateMedals() {
	var sum = 0;
	for (var i = 0; i < maps.length; ++i) {
		var m = new Map(i);
		/*replay.load(i + "best");
		var time = replay.data.length;*/
		//if (userid > -1) {
			var t = playerBests[i];
		//}
		var time;
		if (typeof(t) == 'undefined')
			time = 0;
		else
			time = t[0];
		if (time === 0) time = 1000000;
		if (time <= m.medals.author) {
			rank = 0;
		}
		else if (time <= m.medals.gold) {
			rank = 1;
		}
		else if (time <= m.medals.silver) {
			rank = 2;
		}
		else if (time <= m.medals.bronze) {
			rank = 3;
		}
		else {
			rank = 4;
		}
		medals[i] = rank;
		sum += 4 - rank;
	}
	if (typeof(kongregate) !== 'undefined') {
		kongregate.stats.submit("medals", sum);
	}
}

function escBack() {
	navigateMenu(1);
	gain.gain.value = 0;
}
function mainloop() {
	requestAnimationFrame(mainloop);
	switch(programState) {
		case play: case editPlay:
			player.frame();
			break;
		case editor:
			// something
			break;
	}
	if (mouseDown) {
		if  (lastmd) 
			drag();
		else 
			click();
	}
	
	lastDivPos.x = divPos.x;
	lastDivPos.y = divPos.y;
	lastmd = mouseDown;
	
	checkKeys();
	draw();
	
}

Math.distance = function(a, b) {
	return Math.sqrt(Math.pow(a[0] - b[0], 2) + Math.pow(a[1] - b[1], 2));
}